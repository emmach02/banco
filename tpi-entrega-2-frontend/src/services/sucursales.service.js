...
const getAll = async () => {
    const res = await axios.get(`http://15.229.7.58:4000/api/banco/sucursales`)
    return res.data
}
...
const getSucursal = async (codSucursal) => {
    const res = await axios.get(`http://15.229.7.58:4000/api/banco/sucursales/${codSucursal}`)
    return res.data
}

const deleteSucursal = async (codSucursal) => {
    await axios.delete(`http://15.229.7.58:4000/api/banco/sucursales/${codSucursal}`)
}

const agregarSucursal = async (sucursal) => {
    await axios.post(`http://15.229.7.58:4000/api/banco/sucursales`, sucursal)
}

const actualizarSucursal = async (codSucursal, sucursal) => {
    await axios.put(`http://15.229.7.58:4000/api/banco/sucursales/${codSucursal}`, sucursal)
}
